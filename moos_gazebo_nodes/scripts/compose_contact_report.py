...
class Node(object):
    def __init__(self):
        ...
        self.gateway_key = rospy.get_param("~gateway_key", 'GROUNDTRUTH_CONTACT_REPORT')
        ...
        if self.contact_gt:
            self.s1 = rospy.Subscriber('/' + self.contact_root_namespace + '/' +
                                       ...
                                       NavSatFix,
                                       self.contact_gps_cb)  
            self.s2 = rospy.Subscriber('/' + self.contact_root_namespace + '/' +
                                       self.contact_namespace + '/sensors/gps/gps/fix_velocity',
                                       Vector3Stamped,
                                       self.speed_cb)
            self.s3 = rospy.Subscriber('/' + self.contact_root_namespace + '/' +
    ...
    def contact_groundtruth(self):
        ...
        contact_report.gateway_string = 'NAME=' + self.contact_root_namespace + ',TYPE=ship,TIME=' + str(time_sec) + ',LAT=' + str(self.contact_lat) + ',LON=' + str(self.contact_lon) + ',HDG=' + str(self.contact_heading) + ',YAW=' + str(self.contact_yaw) + ',PITCH=' + str(self.contact_pitch) + ',ROLL=' + str(self.contact_roll) + ',SPD=' + str(self.contact_speed) + ',DEP=' + str(self.contact_depth) + ',LENGTH=' + str(self.contact_length)
